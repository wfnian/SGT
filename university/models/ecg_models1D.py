...
class HeartNet(nn.Module):
    def __init__(
        ...
        self.layer6 = self._make_layer(block, 1024, layers[8], stride=1, dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(conv_subsumpling(self.inplanes, planes * block.expansion), )
    ...
    def forward(self, x):
        ...
        x = self.layer0(x)  # 1024 64 128
        x = self.layer1(x)  # 1024 64 64
        x = self.layer2(x)  # 1024 128 32
        x = self.layer2_(x)  # 1024 128 16
        x = self.layer3(x)  # 1024 256 8
        x = self.layer3_(x)  # 1024 256 4
        x = self.layer4(x)  # 1024 256 2
        x = self.layer4_(x)  # 1024 512 1
        x = x.repeat(1, 1, 2)
        x = self.layer5(x)
...
class Anomaly_Classifier(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.conv = nn.Conv1d(1, out_channels=32, kernel_size=5, stride=1)
    ...
    def forward(self, x):
        ...
        #MLP
        x = x.view(-1, 32 * 4)  #Reshape (current_dim, 32*2)
...
if __name__ == '__main__':
    m = ZolotyhNet()
    m = HeartNetIEEE()
    # m = HeartNet(num_classes=8)
    m = EcgResNet34(num_classes=8)
    # m = Anomaly_Classifier()
    a = torch.rand(16, 128)
    print(m(a).shape)